...
@app.post("/upload")
async def upload(file: UploadFile = File(...), thread_id: str = Form("default")):
    ...
    path = f"uploads/{doc_id}_{file.filename.replace(' ', '_')}"
    with open(path, "wb") as f:
        f.write(await file.read())
    try:
        r = add_document_to_rag(path, thread_id)
        return {"success": True, "doc_id": doc_id, "message": f"Uploaded {r['filename']} ({r['chunks']} chunks)."}
